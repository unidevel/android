...
public class DeviceUtil {
	...
	public static String getGoogleServiceId(Context context) throws Exception {
		...
		Cursor localCursor = context.getContentResolver().query(GSF_URI, null,
				null, arrayOfString, null);
		if (localCursor == null || !localCursor.moveToFirst()
				|| localCursor.getColumnCount() < 2)
			return null;
		try {
			String str = Long.toHexString(Long.parseLong(localCursor
					.getString(1)));
			return str;
		} finally {
			localCursor.close();
	...
	public static void setAndroidId(String androidId) {
		String cmd = SQLITE_PATH
				+ " /data/data/com.android.providers.settings/databases/settings.db "
				+ "\"update secure set value='" + androidId
