...
public class CellWidgetConfigure extends PreferenceActivity implements
		OnClickListener, OnMenuItemClickListener, UmengUpdateListener {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		sidePrefs
				.setOnPreferenceChangeListener(new OnPreferenceChangeListener() {
					public boolean onPreferenceChange(Preference preference,
						...
						if (checked) {
							sidePrefs
									.setSummary(getString(R.string.summary_side_true));
						} else {
							sidePrefs
									.setSummary(getString(R.string.summary_side_false));
						}
						return true;
					}
				});
		...
		String titleSelectApp = String.format(
				getString(R.string.title_select_app), getPrograms());
		...
		for (int i = 0; i < getPrograms(); ++i) {
			String pkg = prefs.getString(getAppPackageKey(mAppWidgetId, i),
					null);
		...
		appListView.getAdapter().registerDataSetObserver(new DataSetObserver() {
			...
			public void onChanged() {
				super.onChanged();
				appPrefs.setSelectedApps(appListView.getApps().toArray(
						new String[0]));
			}
		});
	...
	public void savePref() {
		...
		for (int i = 0; i < appListView.getCount(); i++) {
			edit.putString(getAppPackageKey(mAppWidgetId, i), appListView
					.getAppPackage(i));
			edit.putString(getAppClassKey(mAppWidgetId, i), appListView
					.getAppClass(i));
			edit.putString(getAppIconKey(mAppWidgetId, i), appListView
					.getAppIcon(i));
		}
		edit.commit();
	}
	...
	public void onClick(View v) {
		if (v.getId() == R.id.btn_ok) {
			...
			AppWidgetManager appWidgetManager = AppWidgetManager
					.getInstance(context);
			CellWidget.updateWidget(context, this.getClass(), appWidgetManager,
					mAppWidgetId, getAppWidgetResId());
	...
	private boolean exportSettings(File file) {
		...
		props.setProperty(getTitleColorKey(widgetId), String.valueOf(colorPrefs
				.getColor()));
		props.setProperty(getSideKey(widgetId), String.valueOf(sidePrefs
				.isChecked()));
		props.setProperty(getAppsKey(widgetId), String.valueOf(getPrograms()));
		props.setProperty(getWidgetResIdKey(widgetId), String
				.valueOf(getAppWidgetResId()));
		List<String> apps = appListView.getApps();
		for (int i = 0; i < appListView.getCount(); i++) {
			props.setProperty(getAppPackageKey(widgetId, i), appListView
					.getAppPackage(i));
			props.setProperty(getAppClassKey(widgetId, i), appListView
					.getAppClass(i));
			props.setProperty(getAppIconKey(widgetId, i), appListView
					.getAppIcon(i));
		}
		...
		// props.setProperty(getAppPackageKey(widgetId, i),
		// appPrefs[i].getAppPackage());
		// props.setProperty(getAppNameKey(widgetId, i),
		// appPrefs[i].getAppName());
		// props.setProperty(getAppClassKey(widgetId, i),
		// appPrefs[i].getAppClassName());
		// }
		// }
	...
	private boolean importSettings(File file) {
		...
		try {
			...
			sidePrefs
					.setChecked(toBool(props.get(getSideKey(widgetId)), false));
			colorPrefs
					.setColor(toInt(props.get(getTitleColorKey(widgetId)), 0));
	...
	public boolean onMenuItemClick(MenuItem item) {
		if (item.getItemId() == R.id.menu_export_all
				|| item.getItemId() == R.id.menu_export_icons) {
			...
			builder.setPositiveButton(getString(android.R.string.ok),
					new DialogInterface.OnClickListener() {
						public void onClick(DialogInterface dialog, int which) {
							File file = new File(
									getExportDir(CellWidgetConfigure.this), text
											.getText().toString());
							...
							MobclickAgent.onEvent(CellWidgetConfigure.this,
									"onExportConfig");
						}
					});
			...
			builder.create().show();
		} else if (item.getItemId() == R.id.menu_import_all
				|| item.getItemId() == R.id.menu_import_icons) {
			Builder builder = new Builder(this);
			builder.setTitle(getString(R.string.title_export));
			...
			MobclickAgent.openFeedbackActivity(CellWidgetConfigure.this);
		} else if (item.getItemId() == R.id.menu_upgrade) {
			MobclickAgent.update(CellWidgetConfigure.this);
			...
			Builder builder = new Builder(this);
			String msg = String.format(getString(R.string.msg_remove_app),
					app.name);
			...
			startActivityForResult(Intent.createChooser(intent, ""),
					REQUEST_CHOOSE_IMAGE + info.position);
	...
	protected void onActivityResult(int requestCode, int resultCode, Intent data) {
		...
		if (requestCode >= REQUEST_CHOOSE_IMAGE
				&& requestCode <= REQUEST_CHOOSE_IMAGE + getPrograms()) {
			if (data != null) {
				...
				// File file;
				// file = new File(dir,
				// "icon"+mAppWidgetId+"x"+appIndex+".png");
				// FileOutputStream out = null;
				// try {
				// out = new FileOutputStream(file);
	...
	public void onUpdateReturned(int returnCode) {
		int resIds[] = new int[] { R.string.msg_have_update,
				R.string.msg_no_update, R.string.msg_not_wifi,
				R.string.msg_update_timeout };
